...
void zmq_send_BallDrawDisplayMsg(void * publisher, char ch, int x, int y, int random_secret){
    ...
    ball_draw_display_msg__pack(&pb_m_stuct, (uint8_t*) pb_m_bin);
...
{	
    ...
    PayperviewReq *payperview_req;
    PayperviewResp  pb_m_struct = PAYPERVIEW_RESP__INIT;
    ...
    pb_m_struct.random_secret = random_secret;
    int size_bin_msg = payperview_resp__get_packed_size(&pb_m_struct);
    char * pb_m_bin = malloc(size_bin_msg);
    payperview_resp__pack(&pb_m_struct, (uint8_t*) pb_m_bin);
    while (1)
    {
        int msg_type = 2;
        ...
        if(msg_type == 2){
            ...
            payperview_req = payperview_req__unpack(NULL, msg_len, (uint8_t*) msg_data);
        ...
        if(msg_type == 0){
            ...
            msg_len = zmq_recvmsg (responder, &zmq_msg, 0);
            msg_data = zmq_msg_data (& zmq_msg);
            ClientConnectionReq *client_connection_req = client_connection_req__unpack(NULL, msg_len, msg_data);

            ch = client_connection_req->ch.data[0];
            // TODO 8
        ...
        if(msg_type == 1){
            ...
            msg_len = zmq_recvmsg (responder, &zmq_msg, 0);
            msg_data = zmq_msg_data (& zmq_msg);
            MovementReq *movement_req = movement_req__unpack(NULL, msg_len, msg_data);

            ch = movement_req->ch.data[0];
            direction = movement_req->direction;
            // TODO 11
            int ch_pos = find_ch_info(char_data, n_chars, ch);
            if(ch_pos != -1){
